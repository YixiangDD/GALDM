"""Interpretability case study: VHL/HIF pathway activity in KIRC.

Clear-cell renal carcinoma is characterised by VHL mutation driving aberrant HIF signalling.
This checks whether generated KIRC samples match real ones in HIF-1 pathway activity, and
whether the early/late-stage difference in that activity survives generation.

The pathway masks used here are the same ones the encoder sees, so this is a consistency
check rather than independent biological validation.
"""
import os
import sys
import json
...
def run_interpretability(cancer="KIRC", seed=42, quick=False, vae_ep=None, dit_ep=None):
    ...
    fig.tight_layout()
    fig_path = os.path.join(RESULTS_DIR, f"interpretability_{cancer}_HIF1.png")
    plt.savefig(fig_path, dpi=150); plt.close()

    out = os.path.join(RESULTS_DIR, f"interpretability_{cancer}{'_quick' if quick else ''}.json")
